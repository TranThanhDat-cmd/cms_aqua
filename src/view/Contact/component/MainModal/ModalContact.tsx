...
const ModalContact = (props: IPropsModal) => {
  const { modal, setModal, handleRefresh } = props;
  const [form] = Form.useForm();
  const { formatMessage, intl } = useAltaIntl();
  const detailcontact = useSingleAsync(contactPresenter.getDetailContact);
  ...
  useEffect(() => {
    form.setFieldsValue(detailcontact);
  ...
  const translateFirstKey = 'contact'; //put your translate here
